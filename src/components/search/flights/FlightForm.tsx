...
export default function FlightForm() {
  ...
  const isValid =
    from.trim() && to.trim() && !!depart && (tripType !== "roundtrip" || !!ret);
    ...
    <Card className="border-none shadow-2xl bg-white/90">
      <CardContent className="space-y-5 p-5 rounded-3xl">
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-4">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                id="umrah"
                checked={umrah}
                onCheckedChange={(c) => setUmrah(!!c)}
              />
              <Label htmlFor="umrah" className="text-sm">
                Umrah
              </Label>
            </div>
          ...
          {tripType === "oneway" && (
            <div className="flex flex-col md:flex-row gap-2 items-start justify-between mt-3">
          ...
          {tripType === "roundtrip" && (
            <div className="flex flex-col md:flex-row gap-1 items-start justify-evenly mt-3">
          ...
          {tripType === "multicity" && (
            <div className="flex flex-col md:flex-row gap-2 items-start justify-around mt-3">
              <div className="w-full">
                <PlaceField label="Journey From" value={from} onChange={setFrom} />
              </div>
              ...
              <div>
                <Button
                  variant="secondary"
                  className="mt-6 bg-[#0030ff] text-white rounded-xl"
                  type="button"
                  onClick={() => alert("+ Add Flight clicked")}
                >
